...
def sort_distances(dist_list):
   ...
   dd = [sorted_list[-tranches:]]
   xxx = max(dd[0])
   sample = {}
   sample['top'] = {max([sorted_list[-tranches:]][0]):min([sorted_list[-tranches:]][0])}
   sample['middle'] = {max([sorted_list[tranches:-tranches]][0]):min([sorted_list[tranches:-tranches]][0])}
   sample['bottom'] = {max([sorted_list[0:tranches]][0]): min([sorted_list[0:tranches]][0])}
   return sample
