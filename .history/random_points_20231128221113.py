...
def create_polygon(location_data: list):
    points = location_data.get('all_drawings')[0] 
    pt = points['geometry']['coordinates']

    polygon = folium.Polygon(pt)
    return polygon
...
def polygon_random_points (poly, num_points):
    ...
    while len(points) < num_points:
        random_point = folium.CircleMarker([random.uniform(min_x, max_x), random.uniform(min_y, max_y)]))
