...
if col2.button("Add Random Checkpoints"):
    if map_data.get('all_drawings') is None:
        ...
        st.session_state["markers"] = [folium.Marker(location=[pt.coords[0][0], pt.coords[0][1]], popup='test', icon=folium.Icon(icon='user', prefix='fa')).add_to(map_data) for pt in y]

        st.session_state["testmarkers"] = [folium.Marker(location=[random.randint(37, 38), random.randint(
